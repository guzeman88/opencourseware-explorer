-- Setup script for opencourseware on native PostgreSQL 17
-- Run as: psql -U postgres -h 127.0.0.1 -f setup_native_pg.sql

-- Create user and database
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'ocw') THEN
    CREATE ROLE ocw WITH LOGIN PASSWORD 'ocwpassword';
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_database WHERE datname = 'opencourseware') THEN
    CREATE DATABASE opencourseware OWNER ocw;
  END IF;
END $$;

\connect opencourseware
