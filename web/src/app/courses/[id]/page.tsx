...
import { recordWatch } from "@/lib/api";
...
export default function CoursePage({ params }: CoursePageProps) {
  ...
  // Record watch history 2 s after the user settles on a video
  useEffect(() => {
    if (!token || !course) return;
    if (debounceRef.current) clearTimeout(debounceRef.current);
    debounceRef.current = setTimeout(() => {
      recordWatch(course.id, activeVideoIndex, token).catch(() => {});
    }, 2000);
    return () => {
      if (debounceRef.current) clearTimeout(debounceRef.current);
    };
  }, [activeVideoIndex, course, token]);
